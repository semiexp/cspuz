...
def test_alldifferent(solver: cspuz.Solver) -> None:
...
def test_fold_or(solver: cspuz.Solver) -> None:
    ...
    assert check_equality_expr(actual, Expr(Op.OR, [a[0, 0], a[0, 1], a[1, 0], a[1, 1], b, c, d]))


def test_fold_or_empty() -> None:
    actual = cspuz.fold_or()
    assert check_equality_expr(actual, Expr(Op.BOOL_CONSTANT, [False]))


def test_fold_or_with_true_constant(solver: cspuz.Solver) -> None:
    a = solver.bool_var()
    b = solver.bool_var()
    c = solver.bool_var()
...
def test_fold_and(solver: cspuz.Solver) -> None:
    a = solver.bool_array((2, 2))
    b = solver.bool_var()
    c = solver.bool_var()
    d = solver.bool_var()
    ...
    assert check_equality_expr(actual, Expr(Op.AND, [a[0, 0], a[0, 1], a[1, 0], a[1, 1], b, c, d]))


def test_fold_and_empty() -> None:
    actual = cspuz.fold_and()
    assert check_equality_expr(actual, Expr(Op.BOOL_CONSTANT, [True]))


def test_fold_and_with_false_constant(solver: cspuz.Solver) -> None:
    a = solver.bool_var()
    b = solver.bool_var()
    c = solver.bool_var()
...
def test_count_true(solver: cspuz.Solver) -> None:
    a = solver.bool_array((2, 2))
    b = solver.bool_var()
...
def test_count_true_empty() -> None:
