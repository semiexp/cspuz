import functools
from typing import Any, Generic, Iterable, Iterator, List, Optional, Sequence, Tuple, TypeVar, Union, cast, overload

from .expr import BoolExpr, BoolExprLike, BoolOp, Expr, ExprLike, IntExpr, IntExprLike, IntOp, Op, is_bool_op
...
class Array2D(Generic[T]):
    ...
    def __init__(self, data: Union[Iterable[Iterable[T]], Iterable[T]], shape: Optional[Tuple[int, int]] = None):
        if shape is None:
            data_list_list: List[List[T]] = list(map(lambda e: list(e), cast(Iterable[Iterable[T]], data)))
            ...
            if len(data_list) != size:
                raise ValueError(f'size of `data` ({size}) is inconsistent with the given shape {shape}')
            self.shape = shape
            self.data = data_list
    ...
    def _getitem_impl(self, key: Union[int, slice, Tuple[int, int], Tuple[int, slice], Tuple[slice, int], Tuple[slice, slice]]) -> Union[T, Array1D[T], 'Array2D[T]']:
        if isinstance(key, (int, slice)):
            return self._getitem_impl(cast(Union[Tuple[int, slice], Tuple[slice, slice]], (key, slice(None, None))))
...
ElementwiseOperands = List[Union[BoolExprLike, 'BoolArray1D', 'BoolArray2D', IntExprLike, 'IntArray1D', 'IntArray2D']]
...
@overload
def _elementwise(op: BoolOp, shape: Tuple[int, int], operands: ElementwiseOperands) -> 'BoolArray2D': ...
...
def _elementwise(op: Op, shape: Union[Tuple[int], Tuple[int, int]], operands: ElementwiseOperands) -> Union['BoolArray1D', 'IntArray1D', 'BoolArray2D', 'IntArray2D']:
    # type check
    if op in [Op.EQ, Op.NE, Op.LE, Op.LT, Op.GE, Op.GT]:
        ...
        if len(operands) != 3 or not (_is_bool_like(operands[0]) and _is_int_like(operands[1]) and _is_int_like(operands[2])):
            return NotImplemented
    else:
...
class BoolArray2D(Array2D[BoolExpr]):
    ...
    def __init__(self, data: Union[Iterable[Iterable[BoolExpr]], Iterable[BoolExpr]], shape: Optional[Tuple[int, int]] = None):
    ...
    def __getitem__(self, key: Union[int, slice, Tuple[int, int], Tuple[int, slice], Tuple[slice, int], Tuple[slice, slice]]) -> Union[BoolExpr, BoolArray1D, 'BoolArray2D']:
    ...
    def four_neighbors(self, y: Union[int, Tuple[int, int]], x: Optional[int] = None) -> BoolArray1D:
    ...
    def four_neighbor_indices(self, y: Union[int, Tuple[int, int]], x: Optional[int] = None) -> List[Tuple[int, int]]:
...
class IntArray2D(Array2D[IntExpr]):
    ...
    def __init__(self, data: Union[Iterable[Iterable[IntExpr]], Iterable[IntExpr]], shape: Optional[Tuple[int, int]] = None):
        super().__init__(data, shape)
    ...
    def __getitem__(self, key: Union[int, slice, Tuple[int, int], Tuple[int, slice], Tuple[slice, int], Tuple[slice, slice]]) -> Union[IntExpr, IntArray1D, 'IntArray2D']:
        ret = super()._getitem_impl(key)
        if isinstance(ret, Array1D):
    ...
    def four_neighbors(self, y: Union[int, Tuple[int, int]], x: Optional[int] = None) -> IntArray1D:
...
def _reshape(array: Union[BoolArray1D, BoolArray2D, IntArray1D, IntArray2D], shape: Tuple[int, int]) -> Union[BoolArray2D, IntArray2D]:
...
def _four_neighbors(array: Union[BoolArray2D, IntArray2D], y: Union[int, Tuple[int, int]], x: Optional[int]) -> List[Expr]:
    if x is None:
        if isinstance(y, int):
...
def _four_neighbor_indices(shape: Tuple[int, int], y: Union[int, Tuple[int, int]], x: Optional[int]) -> List[Tuple[int, int]]:
    if x is None:
        if isinstance(y, int):
            raise TypeError('two integers must be provided to \'cell_neighbors\'')
        y2, x2 = y
