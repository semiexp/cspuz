...
def emit_svg(height, width, to_right, to_down, cond_row, cond_col):
    ...
    dwg = svgwrite.Drawing(
        size=(boundary * 2 + cell_size * (width + 2), boundary * 2 + cell_size * (height + 2))
    )

    dotted_grid_style = {"stroke": "gray", "stroke_width": 1, "stroke_dasharray": cell_size / 12}
    ...
    dwg.add(
        dwg.line(
            (boundary - 2 + 2 * cell_size, boundary + (height + 2) * cell_size),
    ...
    dwg.add(
        dwg.line(
            (boundary + 2 * cell_size, boundary - 2 + 2 * cell_size),
    ...
    dwg.add(
        dwg.line(
            (boundary + 2 * cell_size, boundary + cell_size),
    ...
    dwg.add(
        dwg.line(
            (boundary + cell_size, boundary + 2 * cell_size),
    ...
    for y in range(height):
        for x in range(width):
            if y < height - 1 and to_down[y][x]:
                dwg.add(
                    dwg.line(
                        (boundary + (x + 2) * cell_size, boundary + (y + 3) * cell_size),
            ...
            if x < width - 1 and to_right[y][x]:
                dwg.add(
                    dwg.line(
                        (boundary + (x + 3) * cell_size, boundary + (y + 2) * cell_size),
                        (boundary + (x + 3) * cell_size, boundary + (y + 3) * cell_size),
                        **dotted_grid_style
                    )
                )
