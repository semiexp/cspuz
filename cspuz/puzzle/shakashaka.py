    ...
    for y in range(height + 1):
        for x in range(width + 1):
            ...
            if y > 0 and x > 0:
                ...
                is_empty.append(
                    answer[y - 1, x - 1] == 0 if problem[y - 1][x - 1] is None else False
                )
                if problem[y - 1][x - 1] is None:
                    is_white_angle.append(
                        (answer[y - 1, x - 1] == 0) | (answer[y - 1, x - 1] == 1)
                    )
            else:
...
def generate_shakashaka(height, width, verbose=False):
    generated = generate_problem(
        ...
        builder_pattern=ArrayBuilder2D(
            height, width, [None, -1, 0, 1, 2, 3, 4], default=None, disallow_adjacent=True
        ),
            ...
            if problem is not None:
                print(
                    util.stringify_array(
                        problem, {None: ".", -1: "#", 0: "0", 1: "1", 2: "2", 3: "3", 4: "4"}
                    )
                )
