...
def solve_firefly(height, width, problem):
    ...
    solver.ensure(Array(list(has_line)) == (Array(list(line_ul)) | Array(list(line_dr))))
    ...
    for y in range(height):
        for x in range(width):
            ...
            if y > 0:
                adj.append((line_dr.vertical[y - 1, x], line_ul.vertical[y - 1, x], n_turn_vertical[y - 1, x]))
            else:
                adj.append(None)
            if y < height - 1:
                ...
                adj.append(None)
            if x > 0:
                adj.append((line_dr.horizontal[y, x - 1], line_ul.horizontal[y, x - 1], n_turn_horizontal[y, x - 1]))
            else:
                adj.append(None)
            if x < width - 1:
                adj.append((line_ul.horizontal[y, x], line_dr.horizontal[y, x], n_turn_horizontal[y, x]))
            else:
                adj.append(None)
            ...
            if problem[y][x][0] != '.':
                ...
                for i in range(4):
                    if adj[i] is not None and i != out_idx:
                        ...
                        solver.ensure(adj[i][0].then((adj[i][2] == 0) | (adj[i][2] == n_turn_unknown)))
            else:
                adj_present = list(filter(lambda x: x is not None, adj))
                solver.ensure(count_true(map(lambda x: x[0], adj_present)) <= 1)
                solver.ensure(
                    count_true(map(lambda x: x[0], adj_present)) == count_true(map(lambda x: x[1], adj_present)))

                for i in range(4):
                    for j in range(4):
                        if adj[i] is not None and adj[j] is not None and i != j:
                            if (i // 2) == (j // 2):   # straight
                                solver.ensure((adj[i][0] & adj[j][1]).then(
                                    adj[i][2] == adj[j][2]
                                ))
                            else:
                                solver.ensure((adj[i][0] & adj[j][1]).then(
                                    ((adj[i][2] == n_turn_unknown) & (adj[j][2] == n_turn_unknown)) | (adj[i][2] == adj[j][2] + 1)
                                ))
    is_sat = solver.solve()
...
def generate_firefly(height, width, min_clue=0, max_clue=5, verbose=False):
    ...
    for d in ['^', 'v', '<', '>']:
        ...
        for i in range(min_clue, max_clue + 1):
            cand.append(d + str(i))
    generated = generate_problem(lambda problem: solve_firefly(height, width, problem),
