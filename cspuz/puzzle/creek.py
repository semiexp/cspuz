...
def solve_creek(height, width, problem):
    ...
    for y in range(0, height + 1):
        for x in range(0, width + 1):
            if problem[y][x] >= 0:
                solver.ensure(count_true(~is_white[max(y - 1, 0):min(y + 1, height), max(x - 1, 0):min(x + 1, width)]) == problem[y][x])
    is_sat = solver.solve()
...
def generate_creek(height, width, no_easy=False, verbose=False):
    ...
    for y in range(height + 1):
        ...
        for x in range(width + 1):
            ...
            row.append(Choice([-1] + list(range(1 if no_easy else 0, nmax if no_easy else nmax + 1)), default=-1))
        pattern.append(row)
