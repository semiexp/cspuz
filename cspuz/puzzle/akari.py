...
def generate_akari(height, width, no_easy=False, verbose=False):
    def pretest(problem):
        ...
        def visit(y, x):
            if not (
                0 <= y < height and 0 <= x < width and problem[y][x] == -2 and not visited[y][x]
            ):
                return
            visited[y][x] = True
            visit(y - 1, x)
        ...
        for y in range(height):
            for x in range(width):
                if problem[y][x] >= 0:
                    n_adj = (
                        (1 if y > 0 and problem[y - 1][x] == -2 else 0)
                        + (1 if x > 0 and problem[y][x - 1] == -2 else 0)
                        + (1 if y < height - 1 and problem[y + 1][x] == -2 else 0)
                        + (1 if x < width - 1 and problem[y][x + 1] == -2 else 0)
                    )
...
def _main():
    if len(sys.argv) == 1:
        ...
        # fmt: off
        problem = [
            [-2, -2,  2, -2, -2, -2, -2, -2, -2, -2],  # noqa: E201, E241
            [-2, -2, -2, -2, -2, -2, -2, -2,  2, -2],  # noqa: E201, E241
            [-2, -2, -2, -2, -2, -2, -2, -1, -2, -2],  # noqa: E201, E241
            [-1, -2, -2, -2,  3, -2, -2, -2, -2, -2],  # noqa: E201, E241
            [-2, -2, -2, -2, -2, -1, -2, -2, -2, -1],  # noqa: E201, E241
            [ 2, -2, -2, -2,  2, -2, -2, -2, -2, -2],  # noqa: E201, E241
            [-2, -2, -2, -2, -2,  3, -2, -2, -2, -1],  # noqa: E201, E241
            [-2, -2, -1, -2, -2, -2, -2, -2, -2, -2],  # noqa: E201, E241
            [-2,  2, -2, -2, -2, -2, -2, -2, -2, -2],  # noqa: E201, E241
            [-2, -2, -2, -2, -2, -2, -2, -1, -2, -2],  # noqa: E201, E241
        ]
        # fmt: on
        ...
        if is_sat:
            print(util.stringify_array(has_light, {True: "O", False: ".", None: "?"}))
    else:
            ...
            if problem is not None:
                print(
                    util.stringify_array(
                        problem, {-2: ".", -1: "#", 0: "0", 1: "1", 2: "2", 3: "3", 4: "4"}
                    )
                )
                print("", flush=True)
